...
def test_malformed_persisted_mock_roster_blocks_generation_until_valid_save(fleet_cfg, tmp_path):
    ...
    asyncio.run(manager.save_roster(dict(cfg.roster)))
...
def test_bootstrap_persists_only_enabled_template_roles_and_never_guesses(fleet_cfg):
    fleet = fleet_module()
    ...
    assert fleet_cfg.drones[1] not in persisted
    assert store.stack['roster_hash'] is None
    assert store.stack['generation_error']
...
def test_save_roster_is_persistent_refreshes_metadata_and_invalidates_applied_hash(fleet_cfg):
    ...
    asyncio.run(manager.save_roster(replacement))
...
def test_roster_and_stack_mutations_rejected_outside_idle(fleet_cfg, state):
    ...
    for action in (lambda: manager.save_roster(fleet_cfg.roster), manager.start_stack, manager.restart_stack):
...
def test_generation_failure_recovers_without_old_checklist_error(fleet_cfg):
    ...
    assert store.stack['roster_hash'] is None
    fleet_cfg.raw['fleet']['drones'][-1]['uri'] = 'radio://2/80/2M/000000000A'
    assert manager.regenerate() is not None
...
def test_second_spawn_failure_reaps_first_child_and_never_applies_hash(fleet_cfg, tmp_path):
    fleet = fleet_module()
    store = TelemetryStore(fleet_cfg)
    manager = fleet.FleetManager(fleet_cfg, store)
    script = write_dummy(tmp_path)
    ...
    async def run():
        with pytest.raises(OSError):
            await manager.start_stack()
        assert all(child.returncode is not None for child in manager.children.values())
        assert store.stack['applied_hash'] is None
        assert not list(manager.run_dir.glob('*.pid'))
        await manager.close()
    asyncio.run(run())
